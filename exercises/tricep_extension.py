...
class TricepExtensionAnalyzer:
    ...
    def analyze(self, landmarks, image):
        ...
        try:
            ...
            if elbow_angle > 160:
                frame_feedback.append("✅ Full extension!")
            elif elbow_angle > 140:
                frame_feedback.append("✅ Good!")
            else:
                frame_feedback.append("⚠️ Extend fully!")
                frame_score -= 15
    ...
    def _count_reps(self, elbow_angle):
        if elbow_angle > 150 and not self.is_extended:
            self.is_extended = True
            self.rep_count += 1
        elif elbow_angle < 90 and self.is_extended:
            self.is_extended = False
